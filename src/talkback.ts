...
export class Talkback extends TwinkleModule {
	...
	static callback() {
		...
		result.previewer = new Morebits.wiki.preview(
			$(result).find('div#friendlytalkback-previewbox').last()[0]
		);
		...
		var user = new Morebits.wiki.user(
			...
			'Fetching talkback opt-out status'
		);
	...
	static optoutStatus(userobj) {
		...
		if (tl && tl.length) {
			Talkback.optout =
				...
				' transcludes {{' +
				tl[0] +
				'}}, so a talkack might not be helpful';
		} else {
		...
		switch (value) {
			...
			default:
				...
				work_area.append({
					...
					tooltip:
						"The section heading where the discussion is being held. For example: 'Merge proposal'.",
					...
					event: function (e) {
						if (e.target.value === 'afchd') {
							Morebits.quickForm.overrideElementLabel(
								root.section,
								'Title of draft (excluding the prefix): '
							);
	...
	static noticeboards = {
		...
		blpn: {
			...
			editSummary:
				'Notice of discussion at [[Wikipedia:Biographies of living persons/Noticeboard]]',
		},
		coin: {
		...
		th: {
			...
			editSummary:
				'You have replies at the [[Wikipedia:Teahouse/Questions|Teahouse question board]]',
		},
		otrs: {
	...
	static evaluate(e) {
		...
		Morebits.wiki.actionCompleted.notice =
			'Talkback complete; reloading talk page in a few seconds';
		...
		switch (input.tbtarget) {
			...
			default:
				...
				input.page = Talkback.callbacks.normalizeTalkbackPage(input.page);
				talkpage.setEditSummary(
					'Talkback ([[:' + input.page + (input.section ? '#' + input.section : '') + ']])'
				);
				break;
		}
			...
			switch (input.tbtarget) {
				case 'notice':
					text = Morebits.string.safeReplace(
						Talkback.noticeboards[input.noticeboard].text,
						'$SECTION',
						input.section
					);
					break;
