...
export class Shared extends TwinkleModule {
	...
	constructor() {
		super();
		if (
			mw.config.get('wgNamespaceNumber') === NS_USER_TALK &&
	...
	static standardList = [
		...
		{
			label:
				'{{Dynamic IP}}: shared IP address template modified for organizations with dynamic addressing',
			value: 'Dynamic IP',
		},
		{
		...
		{
			label:
				'{{Mobile IP}}: shared IP address template modified for mobile phone companies and their customers',
			value: 'Mobile IP',
		},
		{
			label:
	...
	static callbacks = {
		main: function (pageobj) {
			...
			for (var i = 0; i < Shared.standardList.length; i++) {
				...
				if (tagRe.exec(pageText)) {
					Morebits.status.warn(
						'Info',
						'Found {{' +
							Shared.standardList[i].value +
							"}} on the user's talk page already...aborting"
					);
					found = true;
				}
			}
			...
			Morebits.status.info(
				'Info',
				"Will add the shared IP address template to the top of the user's talk page."
			);
			var text = Shared.getTemplateWikitext(params);

			var summaryText =
				'Added {{[[Template:' + params.template + '|' + params.template + ']]}} template.';
