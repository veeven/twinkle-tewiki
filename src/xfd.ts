...
class Afd extends XfdMode {
	...
	evaluate() {
		...
		this.tm.add(
			this.createDiscussionPage,
			[this.checkPage, this.determineDiscussionPage],
			this.printReasonText
		);
			...
			if (!pageobj.exists()) {
				...
				if (text === old_text) {
					...
					linknode.setAttribute(
						'href',
						mw.util.getUrl('Wikipedia:Twinkle/Fixing AFD') + '?action=purge'
					);
...
class Tfd extends XfdMode {
	...
	generateFieldset(): Morebits.quickForm.element {
		this.fieldset = super.generateFieldset();
		var templateOrModule =
			mw.config.get('wgPageContentModel') === 'Scribunto' ? 'module' : 'template';
		this.fieldset.append({
			type: 'select',
			...
			list: [
				{
					...
					name: 'devpages',
					tooltip:
						'A notification template will be sent to Twinkle, AWB, and RedWarn if this is true.',
					checked: true,
				},
			],
		});
	...
	tagPagesForMerge() {
		...
		let pageobj = new Page(
			`${Morebits.pageNameNorm}${docOrNot}`,
			...
			obj_entries(categoryNotificationPageMap)
				...
				.map(([cat, page]) => {
					return this.notifyTalkPage(
						page,
						new Morebits.status('Notifying ' + page + ' of template nomination')
					);
				})
		);
	}
...
class Cfd extends XfdMode {
	...
	tagPage() {
		...
		var pageobj = new Page(
			...
			'Tagging category with ' + params.action + ' tag'
		);
		pageobj.setFollowRedirect(true); // should never be needed, but if the page is moved, we would want to follow the redirect
		return pageobj.load().then(() => {
	...
	addToList() {
		...
		return pageobj.load().then(() => {
			...
			pageobj.setPageText(text);
			pageobj.setEditSummary(
				'Adding ' + params.action + ' nomination of [[:' + Morebits.pageNameNorm + ']].'
			);
			pageobj.setWatchlist(getPref('xfdWatchDiscussion'));
			pageobj.setCreateOption('recreate');
			return pageobj.save();
...
class Cfds extends XfdMode {
	...
	tagPage() {
		...
		return pageobj.load().then(() => {
			var text = pageobj.getPageText();
			params.tagText =
				'{{subst:cfr-speedy|1=' + params.cfdstarget.replace(/^:?Category:/, '') + '}}\n';
			if (pageobj.canEdit()) {
				pageobj.setPageText(params.tagText + text);
				pageobj.setEditSummary(
					'Listed for speedy renaming; see [[WP:CFDS|Categories for discussion/Speedy]].'
				);
				pageobj.setWatchlist(getPref('xfdWatchPage'));
				pageobj.setCreateOption('recreate'); // since categories can be populated without an actual page at that title
				return pageobj.save();
...
class Mfd extends XfdMode {
	...
	addToList() {
		...
		return pageobj.load().then(() => {
			...
			var date_header_regex = new RegExp(
				date.format('(===[\\s]*MMMM[\\s]+D,[\\s]+YYYY[\\s]*===)', 'utc')
			);
...
class Rfd extends XfdMode {
			...
			list: [
				{
					...
					name: 'relatedpage',
					tooltip:
						"A notification template will be placed on the talk page of this redirect's target if this is true.",
					checked: true,
				},
			],
		});
	...
	tagPage(): JQuery.Promise<void> {
		...
		return pageobj.load().then(() => {
			...
			params.tagText =
				'{{subst:rfd|' +
				(mw.config.get('wgNamespaceNumber') === 10 ? 'showontransclusion=1|' : '') +
				'content=\n';
...
class Rm extends XfdMode {
	...
	listAtTalk() {
		...
		pageobj.setFollowRedirect(true);
		pageobj.setEditSummary(
			'Proposing move' + (params.newname ? ' to [[:' + params.newname + ']]' : '')
		);
