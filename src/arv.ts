		...
		switch (value) {
			...
			default:
				...
				work_area.append({
					...
					disabled:
						!mw.util.getParamValue('vanarticle') || mw.util.getParamValue('vanarticlerevid'),
				});
				work_area.append({
					type: 'checkbox',
					name: 'arvtype',
					...
					event: function (e) {
						...
						var getAN3Entries = function (field, rvuser, titles) {
							...
							new mw.Api()
								...
								.fail(function () {
									$('<span class="entry">API failure, reload page and try again</span>').appendTo(
										$field
									);
								});
						};
		...
		switch (form.category.value) {
						...
						if (
							...
							if (
								confirm(
									'The user ' +
										uid +
										' has already been reported by a bot. Do you wish to make the report anyway?'
								)
							) {
			...
			case 'an3':
				...
				if (
					diffs.length < 3 &&
					!confirm(
						'You have selected fewer than three offending edits. Do you wish to make the report anyway?'
					)
				) {
					return;
				}
	...
	static processSock(params) {
		...
		var text =
			...
			params.sockpuppets
				.map(function (v) {
					return (
						'* {{' + (mw.util.isIPAddress(v, true) ? 'checkip' : 'checkuser') + '|1=' + v + '}}'
					);
				})
				.join('\n') +
			'\n|evidence=' +
	...
	static processAN3(params) {
		...
		new mw.Api()
			...
			.done(function (data) {
				...
				if (orig) {
					origtext =
						'{{diff2|' + orig.revid + '|' + orig.timestamp + '}} ' + hasHiddenComment(orig);
				}
