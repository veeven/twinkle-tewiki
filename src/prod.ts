...
export class Prod extends TwinkleModule {
		...
		field.append({
			...
			list: [
				...
				},
				{
					...
					value: 'prodblp',
					tooltip:
						'Proposed deletion of new, completely unsourced biographies of living persons, per [[WP:BLPPROD]]',
				},
			],
		});
	...
	prodtypechanged(event) {
					...
					list: [
						{
							...
							name: 'notify',
							tooltip:
								"A notification template will be placed on the creator's talk page if this is true.",
							checked: true,
						},
					],
				});
		...
		event.target.form.replaceChild(
			field.render(),
			$(event.target.form).find('fieldset[name="parameters"]')[0]
		);
	}
			...
			if (!prod_re.test(text)) {
				...
				if (params.oldProdPresent) {
					if (params.blp) {
						if (
							!confirm(
								'Previous PROD nomination found on talk page. Do you still want to continue applying BLPPROD? '
							)
						) {
				...
				summaryText =
					'Endorsing proposed deletion per [[WP:' +
					(params.blp ? 'BLP' : params.book ? 'BOOK' : '') +
					'PROD]].';
				text = text.replace(
					prod_re,
					text.match(prod_re) +
	...
	evaluate(e) {
		...
		if (!this.params.blp && !this.params.reason) {
			if (
				!confirm('You left the reason blank, do you really want to continue without providing one?')
			) {
				return;
			}
		}
